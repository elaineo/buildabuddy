...
def start_it_up(logger, bot_api, username, botname, userid, bot_id_str, delay=0, num_topics=8, debug=False, custom_garbage=None, custom_clean=None):
    ...
    # get friends
    friends_raw = bot_api.request('friends/ids', {'screen_name': username})
    ...
    for tweet in r:
        ...
        try:
            ...
            if sleep or wake or debug or end_debug:
                twit = bot_api.request('statuses/update', {'status': '@%s yes boss' % username, 'in_reply_to_status_id': id_str})
            if sleep:
                asleep = True
            if wake:
                asleep = False
            ...
            for page in pages:
                ...
                if len(sims) > 4:
                    ...
                    sample = sents
                sample = [s for s in sample if "subscribe at this link" not in s]
